...
#import "YGCommon.h"
@interface YGSegmentControl()<UIScrollViewDelegate>
...
- (void)segmentItemClicked:(YGSegmentItem *)item;

@end
@implementation YGSegmentControl
...
- (void)layoutSubviews
{
    ...
    //  contentSize
    YGSegmentItem *item = [self.items lastObject];
...
- (void)createItems
{
    ...
    for (int i = 0; i < self.titles.count; i++) {
        ...
        if (self.segmentType == XHSegmentTypeFilled) {
            ...
            itemWidth = [YGSegmentItem caculateWidthWithtitle:title titleFont:self.titleFont];
            YGSegmentItem *lastItem = [arrayItem lastObject];
        ...
        YGSegmentItem *item = [self createItem:itemRect title:title];
...
- (YGSegmentItem *)createItem:(CGRect)rect title:title
{
    YGSegmentItem *item = [[YGSegmentItem alloc] initWithFrame:rect];
    item.title = title;
...
- (void)scrollToRate:(CGFloat)rate
{
    ...
    YGSegmentItem *currentItem = self.items[self.selectIndex];
    YGSegmentItem *previousItem = self.selectIndex > 0 ? self.items[self.selectIndex - 1]: nil;
    YGSegmentItem *nextItem = (self.selectIndex < self.items.count - 1)? self.items[self.selectIndex + 1]: nil;
    if (fabs(rate) > 0.5) {
...
- (void)segmentItemSelected:(YGSegmentItem *)item
{
    for (YGSegmentItem *i in self.items) {
...
- (void)setSelectIndex:(NSInteger)selectIndex animation:(BOOL)animation
{
    ...
    if (selectIndex < self.items.count) {
        ...
        YGSegmentItem *item = self.items[selectIndex];
