...
#import "YGCommon.h"
@interface YGAddAlbumVC ()<UITextFieldDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
{
...
- (void)addSubViews{
    ...
    __block YGAddAlbumVC *weakself  = self;
    _collectionView.addBlock = ^{
        NSLog(@"++");
        
        [weakself openMenu];
    };
    _collectionView.delectPhotoBlock = ^(NSInteger index){
        NSLog(@"--");
...
- (void)confirmAction{
    ...
    if ([_nameTXF.text length] ==0) {
        ...
        [parameter setObject:@"photoAdd" forKey:@"class_id"];
        [parameter setObject:@"photoAdd" forKey:@"title"];
        [parameter setObject:@"photoAdd" forKey:@"imgs"];
        [YGNetWorkManager setUpAlbumWithParameter:parameter completion:^(id responseObject) {
